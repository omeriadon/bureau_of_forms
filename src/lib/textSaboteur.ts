...
export function startTextSaboteur({
	freq = 2100,
	chance = 0.32,
} = {}): Stopper {
	...
	const sel =
		"p, li, h1, h2, h3, span, label, button, a, small, .content-copy, .content-title";
	...
	const id = window.setInterval(
		() => {
			try {
				...
				for (
					let i = 0;
					i < Math.max(4, Math.floor(all.length * 0.06));
					i++
				) {
					...
					if (Math.random() < chance) {
						...
						if (r < 0.2)
							...
							el.textContent =
								shuffleWords(el.textContent || "") ||
								randomGibberish(
									4 + Math.floor(Math.random() * 6),
								);
						else
							el.textContent = randomGibberish(
						...
						if (txt.length > 5 && Math.random() < 0.3) {
							const pos = Math.floor(
								Math.random() * Math.min(6, txt.length),
							);
							el.textContent =
								txt.slice(0, pos) +
								randomGibberish(2) +
